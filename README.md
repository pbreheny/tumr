...
either t-test, ANOVA, Tukey, or both ANOVA and Tukey. rfeat() has a plot() 
function that shows the individual beta coefficients and mean slope by treatment 
group. 
...
is centered. lmm() includes a summary() function that provides the overall effect
...
slope differences between the treatment groups. lmm() also includes a plot() 
function that shows the predicted values of the tumor growth measurements over time 
