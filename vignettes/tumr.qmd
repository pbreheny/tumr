...
- Note that the function used to create a `tumr` object includes a built-in mechanism to check the scale of the time variable. This is because poor scaling of the time variable is a common cause of convergence issues when fitting models with `lmm()`. A simple and effective solution is to rescale time to a larger unit (for example, from days to months). Users can refer to this [article](articles/troubleshooting.html) for more details.
