...
The figure below compares a naive longitudinal visualization with a median-based approach that explicitly accounts for censoring and missing observations.
...
For datasets that exhibit non-linear growth, we also provide the [Exponential quadratic model](articles/quadratic.html) and [Generalized Addictive Model (GAM)](articles/gam.html)
...
In addition to the linear mixed-effects modeling, our package also supports fitting a Bayesian hierarchical linear model. Detailed usage of this model is described in an [article](articles/bhm.html).
